...
| ~~Manifests — create/mutate only, no read-back, no status transition~~ | — | **Resolved.** See Phase 7 note below. |
...
**Status:** Resolved — Memberships, Organizations, Manifests, and Storage all done.
...
- **Manifests — resolved.** `GET /api/manifests/{id}` and `GET /api/manifests` (paginated, org-membership-gated via a new `Manifest` case in `ResourceAuthorizationHandler`, following `GetItemAuditTrail`'s `page`/`pageSize`/`X-Total-Count`/404/403 pattern) plus `PATCH /api/manifests/{id}/status` (Owner-role member or admin/super_admin only; enforces a linear state machine — `Draft→Pending→InTransit→Received`, `Cancelled` reachable from `Draft`/`Pending`/`InTransit`) shipped. `Manifest.OrganizationId` is now required (migration `MakeManifestOrganizationIdRequired`) and `CreateManifest` now verifies caller org-membership, which it didn't before. Does not change §2.8's Worker/Api no-FK posture. See `src/ToTen.Api/Features/Manifests/`, `tests/ToTen.Api.IntegrationTests/Manifests/ManifestsEndpointsTests.cs`.
- **Storage — resolved.** `Box` CRUD (`Features/Storage/CreateBox/`, `GetBox/`, `GetBoxes/`, `UpdateBox/`, `DeleteBox/`) shipped, reusing the `ResourceOwnerRequirement`/`ResourceAuthorizationHandler` pattern (its `Box` and `Location` cases already existed but were previously unreferenced by any endpoint). `Location` gained `GetLocation`/`GetLocations`/`UpdateLocation`/`DeleteLocation` to pair with the existing create-only endpoint. Two adjacent fixes landed in the same pass since they touched the same file/entities: `CreateLocation` now verifies caller org-membership before create (it didn't before, unlike `CreateManifest`) and both `CreateLocation`/`UpdateLocation` pre-check the `(OrganizationId, Name)` unique index to return `409` instead of an unhandled `DbUpdateException`. `DeleteLocation`/`DeleteBox` return `409 Conflict` instead of deleting when Boxes/Items/Manifests still reference them — `Box.LocationId` is a required FK with no explicit `OnDelete` (EF default `Cascade`), which would otherwise silently cascade-delete a Location's Boxes; this brings Location's delete safety in line with `Manifest`'s explicit `Restrict` posture on its own Location FKs. See `src/ToTen.Api/Features/Storage/`, `tests/ToTen.Api.IntegrationTests/Storage/LocationEndpointsTests.cs`, `tests/ToTen.Api.IntegrationTests/Storage/BoxEndpointsTests.cs`.
  - **Deferred, not fixed in this pass:** `MoveItem` (Storage), `AssociateBoxes`/`GenerateQR` (Manifests) still gate Box/Location access with a raw `resource.OwnerId == user.Id` string comparison, ignoring org membership entirely — an org-shared Box/Location is inaccessible to teammates through those three flows even though the new Box/Location CRUD endpoints (and Items/Manifests' own Get/Update/Delete) correctly use `ResourceAuthorizationHandler`. Scoped out because those three endpoints belong to Items/Manifests slices from prior phases; upgrading them is a small, mechanical follow-up (swap the manual check for `authorizationService.AuthorizeAsync(principal, resource, new ResourceOwnerRequirement())`) whenever that area is next touched.
