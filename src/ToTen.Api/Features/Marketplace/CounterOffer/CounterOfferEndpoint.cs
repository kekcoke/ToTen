...
public static class CounterOfferEndpoint
{
    public static void MapCounterOffer(this IEndpointRouteBuilder app)
    {
        app.MapPost("/api/offers/{offerId:guid}/counter", async (
            Guid offerId,
            ...
            ClaimsPrincipal principal) =>
        {
            ...
            if (offer.Status != OfferStatus.Pending) return Results.BadRequest("Only pending offers can be countered.");

            offer.Status = OfferStatus.Countered;
            offer.CounterAmount = request.CounterAmount;
